...
def make_station_climate_plots_pairs(
    station_climate_df: pd.DataFrame,
    output_dir: str,
    climate_col: str = 'Climate_Abbreviation',
) -> None:
    """
    Create plots for the climate data of each station with both violin and KDE plots. Plots are created in pairs of variables.
    ...
    os.makedirs(output_dir, exist_ok=True)
    station_vars = [
        'TMax (C)', 'TAvg (C)', 'TMin (C)', 'Ea (kPa)', 'TDew (C)',
        ...
        'ETo (mm/day)', 'ETr (mm/day)'
    ]
    station_climate_df = station_climate_df[station_climate_df[climate_col].astype(str) != 'None']
    climate = sorted(station_climate_df[climate_col].unique())
    ...
    for group_idx, var_group in enumerate(var_groups):
        ...
        if len(var_group) == 2 and all('ET' in var for var in var_group):
            ...
            for var in var_group:
                var_data = station_climate_df[var].dropna()
                if len(var_data) > 0:
        ...
        for var_idx, var in enumerate(var_group):
            ...
            overall_data = station_climate_df[var].dropna()
            ...
            if var in ['TMax (C)', 'TMin (C)', 'TAvg (C)', 'TDew (C)']:
        ...
        n_total = len(station_climate_df)
        labels.append(f'All sites ({n_total:,})')
        
        # Get crop colors for legend
        for climate_val in climate:
            n_climate = len(station_climate_df[station_climate_df[climate_col] == climate_val])
